...
export function AlumniDetailsForm() {
  ...
  const [jobs, setJobs] = useState<{ data: Partial<JobPositionData>; ongoing: boolean }[]>([
    { data: {}, ongoing: false },
  ]);

  const [education, setEducation] = useState<{ data: Partial<EducationData>; ongoing: boolean }[]>([
    { data: {}, ongoing: false },
  ]);
    const validatePayload = (payload: AlumniDetailsPayload): { isValid: boolean; errors: string[] } => {
    const errors: string[] = [];
    ...
    if (!Array.isArray(payload.jobPosition) || payload.jobPosition.length === 0) {
      ...
      payload.jobPosition.forEach((job, index) => {
        ...
        if (!job.ongoing && !job.end) errors.push(`Job ${index + 1}: End date is required when not ongoing`);
      });
    }
    ...
    if (!Array.isArray(payload.education) || payload.education.length === 0) {
      ...
      payload.education.forEach((edu, index) => {
        ...
        if (!edu.fieldOfStudy) errors.push(`Education ${index + 1}: Field of study is required`);
        if (!edu.start) errors.push(`Education ${index + 1}: Start date is required`);
        if (!edu.location) errors.push(`Education ${index + 1}: Location is required`);
        // Check if end date is required when not ongoing
        if (!edu.ongoing && !edu.end) errors.push(`Education ${index + 1}: End date is required when not ongoing`);
      });
    }
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const jobPosition: JobPositionData[] = jobs.map((job, index) => {
      ...
      const type = formData.get(`jobType-${index}`)?.toString() as JobPositionData["type"];
      const startDate = formData.get(`jobStart-${index}`)?.toString();
      const endDate = job.ongoing ? null : formData.get(`jobEnd-${index}`)?.toString();
      const location = formData.get(`jobLocation-${index}`)?.toString().trim();
      const jobType = formData.get(`workMode-${index}`)?.toString() as JobPositionData["jobType"];
      
      if (!title || !type || !startDate || !location || !jobType) {
        throw new Error(`All fields are required for job position ${index + 1}`);
      }
      ...
      const jobData: JobPositionData = {
        ...
        location,
        jobType,
        description: formData.get(`jobDescription-${index}`)?.toString().trim() || undefined
      };
      return jobData;
    });
    ...
    const educationData: EducationData[] = education.map((edu, index) => {
      ...
      const fieldOfStudy = formData.get(`fieldOfStudy-${index}`)?.toString().trim();
      const startDate = formData.get(`educationStart-${index}`)?.toString();
      const endDate = edu.ongoing ? null : formData.get(`educationEnd-${index}`)?.toString();
      const location = formData.get(`educationLocation-${index}`)?.toString().trim();
      ...
      const eduData: EducationData = {
        ...
        ongoing: edu.ongoing,
        location,
        description: formData.get(`educationDescription-${index}`)?.toString().trim() || undefined
      };
      return eduData;
    });
  ...
  const alumniMutation = useMutation({
    ...
    onError: (error: any) => {
      ...
      toast.error(error.response?.data?.message || "Alumni details submission failed");
  ...
  const addNewJob = () => {
    ...
    if (!allFieldsFilled) {
      setAlertMessage("Please fill in all required fields in the current job entry before adding a new one.");
      setJobAlert(true);
  ...
  const addNewEducation = () => {
    ...
    if (!allFieldsFilled) {
      setAlertMessage("Please fill in all required fields in the current education entry before adding a new one.");
  ...
  return (
    <div className="mt-8 mb-8 max-w-3xl w-full mx-auto border border-gray-300 dark:border-gray-700 rounded-lg p-6 md:p-8 shadow-lg bg-white dark:bg-black">
      ...
      <form onSubmit={handleSubmit} ref={formRef} className="space-y-12"> {/* Increased space between sections */}
        <div className="space-y-8"> {/* Increased space between major sections */}
          
          {/* Job Sections */}
          <div className="space-y-8">
            <div className="flex items-center justify-between border-b border-gray-200 dark:border-gray-700 pb-4">
              <h3 className="font-bold text-lg text-neutral-800 dark:text-neutral-200">Job Positions</h3>
              <Button
                type="button"
                onClick={addNewJob}
            ...
            <div className="space-y-8"> {/* Space between job entries */}
              {Array.isArray(jobs) && jobs.map((job, index) => (
                <div key={index} className="relative border border-gray-200 dark:border-gray-700 rounded-lg p-6 space-y-6">
                  {jobs.length > 1 && (
                    <Button
                      type="button"
                  ...
                  <div className="space-y-6"> {/* Space between fields within a job entry */}
                    <LabelInputContainer>
                    ...
                    <div className="flex flex-col md:flex-row gap-6"> {/* Increased gap between side-by-side fields */}
                      ...
                      <LabelInputContainer className="w-full">
                        ...
                        <Input
                          ...
                          className={job.ongoing ? "bg-gray-200 cursor-not-allowed dark:bg-gray-700" : ""}
                        />
                      </LabelInputContainer>
                    </div>
                    ...
                    <LabelInputContainer>
                      <Label htmlFor={`jobLocation-${index}`}>Job Location</Label>
                      <Input
          ...
          {/* Education Sections */}
          <div className="space-y-4 pt-2"> {/* Added top padding for section separation */}
            <div className="flex items-center justify-between border-b border-gray-200 dark:border-gray-700 pb-4">
              <h3 className="font-bold text-lg text-neutral-800 dark:text-neutral-200">Education</h3>
              <Button
                type="button"
            ...
            <div className="space-y-8"> {/* Space between education entries */}
              {Array.isArray(education) && education.map((edu, index) => (
                <div key={index} className="relative border border-gray-200 dark:border-gray-700 rounded-lg p-6 space-y-6">
                  ...
                  <div className="space-y-6"> {/* Space between fields within an education entry */}
                    <LabelInputContainer>
                    ...
                    <LabelInputContainer>
                      <Label htmlFor={`educationLocation-${index}`}>Location</Label>
                      <Input
                    ...
                    <div className="flex flex-col md:flex-row gap-6">
                      <LabelInputContainer className="w-full">
                        <Label htmlFor={`educationStart-${index}`}>Start Date</Label>
                        <Input
                      ...
                      <LabelInputContainer className="w-full">
                        <Label htmlFor={`educationEnd-${index}`}>End Date</Label>
                        <Input
                          id={`educationEnd-${index}`}
                          name={`educationEnd-${index}`}
                          type="date"
                          disabled={edu.ongoing}
                          className={edu.ongoing ? "bg-gray-200 cursor-not-allowed dark:bg-gray-700" : ""}
                        />
                      </LabelInputContainer>
                    </div>
                    ...
                    <LabelInputContainer>
                      <div className="flex items-center gap-2 mt-2">
                        ...
                        />
                        <Label htmlFor={`ongoingEducation-${index}`}>Ongoing</Label>
                      </div>
                    </LabelInputContainer>

                    <LabelInputContainer>
                      <Label htmlFor={`educationDescription-${index}`}>Education Description</Label>
                      <textarea
          ...
          {/* Expertise Section */}
          <div className="pt-2 space-y-2"> {/* Added padding and spacing */}
            ...
            <div className="flex flex-wrap gap-2 mt-2">
              {Array.isArray(expertise) && expertise.map((skill, index) => (
                <span key={index} className="bg-gray-200 px-3 py-1 rounded-md flex items-center">
                  {skill}
                  <button
                    type="button"
          ...
          >
            {alumniMutation.isPending ? "Submitting..." : "Submit Details"} &rarr;
          </Button>

          {alumniMutation.isError && (
            <p className="text-red-500 text-sm mt-2">
              {alumniMutation.error?.response?.data?.message || "Submission failed"}
            </p>
          )}
        </div>
      </form>
