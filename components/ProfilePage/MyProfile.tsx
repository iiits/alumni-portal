...
const MyProfile: React.FC = () => {
  ...
  return (
    <div className="w-full max-w-[95%] mx-auto p-6 bg-white shadow-lg rounded-2xl flex flex-col space-y-6">
      ...
      {data.role === "student" && (
        <button className="px-4 py-2 bg-black text-white rounded-lg">Register as Alumni</button>
      )}
      {/* Social Media Links */}
          ...
          data?.profiles.map(
            (profile, index) =>
              profile.visibility === "yes" && (
                <Link key={index} href={profile.link} target="_blank" rel="noopener noreferrer">
      ...
      <div className="space-y-3 border-t pt-4">
        <p><Mail className="w-5 h-5" /> College Email: {data?.collegeEmail}</p>
        <p><Mail className="w-5 h-5" /> Personal Email: {data?.personalEmail}</p>
      </div>
      ...
      {data.role === "alumni" && data.alumniDetails && (
        <div className="space-y-4 border-t pt-4">
          {data.alumniDetails.verified === false && (
            <p className="text-yellow-600 font-semibold">Alumni details verification in progress</p>
          )}
          <ModalProvider>
            <EditAlumniDetails
              alumniData={{
          ...
          <p className="text-lg font-semibold">Location: {data.alumniDetails.location.city}, {data.alumniDetails.location.country}</p>
          <div>
            ...
            {data.alumniDetails.jobPosition.map((job, index) => (
              <div key={index} className="pl-4 border-l-2 border-gray-300">
                ...
                <p className="text-gray-500">{job.ongoing ? "Ongoing" : `${job.start.split('T')[0]} to ${job.end ? job.end.split('T')[0] : "Present"}`}</p>
          ...
          <div>
            ...
            {data.alumniDetails.education.map((edu, index) => (
              <div key={index} className="pl-4 border-l-2 border-gray-300">
                ...
                <p className="text-gray-500">{edu.ongoing ? "Ongoing" : `${edu.start.split('T')[0]} to ${edu.end.split('T')[0]}`}</p>
                <p className="text-gray-500">{edu.description}</p>
              </div>
            ))}
          </div>
          <p className="text-gray-700">Expertise: {data.alumniDetails.expertise?.join(", ") || "N/A"}</p>
        </div>
      )}
    </div>
  );
