...
<Dialog>
  <DialogTrigger asChild>
    <Button>
      <span className="mr-1">+</span> Create Referral
    </Button>
  </DialogTrigger>
  <DialogContent>
    <DialogHeader>
      <DialogTitle>Create a New Referral</DialogTitle>
    </DialogHeader>
    {/* Form fields go here */}
  </DialogContent>
</Dialog>
      ...
      <TabsContent value="all">
        <Card>
          <CardHeader>
            ...
            <CardDescription>Browse and filter job referrals.</CardDescription>
          </CardHeader>
          <CardContent>
            {/* Filters */}
            ...
            <ul className="space-y-4">
              {data?.map((referral) => (
                <li key={referral.id} className="p-4 border rounded-lg shadow">
                  <h2 className="text-xl font-semibold">{referral.jobDetails.title}</h2>
                  <p className="text-gray-600">{referral.jobDetails.company}</p>
                  <p className="text-gray-800">{referral.jobDetails.description}</p>
                  <p>
                    <strong>Posted By:</strong> {referral.postedBy.name} ({referral.postedBy.collegeEmail})
                  </p>
                  <p>
                    <strong>Last Apply Date:</strong> {new Date(referral.lastApplyDate).toDateString()}
                  </p>
                  <a
      ...
      <TabsContent value="mine">
        <Card>
          <CardHeader>
            ...
            <CardDescription>Your submitted referrals will appear here.</CardDescription>
          </CardHeader>
          <CardContent>
